...
const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "grid w-full auto-rows-[14rem] grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6",
        className,
      )}
    >
      {children}
...
const BentoCard = ({
  ...
  return (
    ...
    >
      ...
      <div>{background}</div>
      <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-3 p-6 md:p-8 transition-all duration-300 group-hover:-translate-y-4">
        <Icon className={cn("h-10 w-10 md:h-11 md:w-11 origin-left transform-gpu transition-all duration-300 ease-in-out group-hover:scale-90", iconColor)} />
        <h3 className="text-xl md:text-2xl font-semibold text-foreground tracking-tight">{name}</h3>
