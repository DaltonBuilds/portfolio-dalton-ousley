...
/**
 * Gets HMAC client secret from environment
 *
 * The client secret is used to verify the signature generated by the
 * frontend. This value is safe to be public (it is embedded in the
 * frontend) but we still pass it via environment variables for
 * configurability. No network calls or caching required.
 */
export async function getHMACClientSecret(): Promise<string> {
  const secret = process.env.HMAC_CLIENT_SECRET

  if (!secret || secret.length === 0) {
    throw new Error("HMAC client secret not configured (HMAC_CLIENT_SECRET)")
  }
