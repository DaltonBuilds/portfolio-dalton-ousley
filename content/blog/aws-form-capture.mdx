...
projectCategory: "Cloud Engineering"
projectStatus: "Completed"
techStack: ["AWS Lambda", "DynamoDB", "API Gateway", "EventBridge", "Secrets Manager", "CloudWatch", "Terraform", "React", "Cloudflare Turnstile", "Zod"]
projectUrl: "https://daltonousley.com"
githubUrl: "https://github.com/DaltonBuilds/aws-serverless-form-backend"
...
**📦 Full Source Code:** The complete infrastructure code, Lambda functions, and demo application are available on GitHub at [github.com/DaltonBuilds/aws-serverless-form-backend](https://github.com/DaltonBuilds/aws-serverless-form-backend)
...
At a high level, the flow involves **six AWS services** working together in a secure and modular fashion:

- 🚪 **API Gateway** – Serves as the entry point for form submissions with built-in throttling
...
flowchart TB
    ...
    Turnstile -->|3. Token + Payload| APIGW
    APIGW -->|4. Invoke| Lambda1

    Lambda1 <-->|Get Secrets| Secrets
    Lambda1 -->|5. Store Contact| DDB
...
3. The browser includes an idempotency key and Turnstile token in request headers.
4. **API Gateway** receives the request, applies throttling, and invokes the Contact Processor Lambda.
5. **Contact Processor Lambda**:
...
6. **EventBridge** asynchronously triggers the Email Notifier Lambda.
7. **Email Notifier Lambda** uses the Resend API to deliver a notification to my inbox.
8. **CloudWatch** captures logs, metrics, and alarms for the entire flow.
...
sequenceDiagram
    ...
    T->>U: 2. Return token
    U->>A: 3. POST /contacts with headers
    A->>L1: 4. Invoke Lambda
    L1->>L1: 5. Validate headers
...
Each submission request includes metadata headers that support robust server-side validation and deduplication. When the user submits the form:

1. A timestamp is generated.
2. The payload is serialized and hashed with SHA-256.
3. A unique idempotency key is generated client-side.
4. Headers are sent along with the form body to the API Gateway endpoint.

The backend validates the request up front, ensuring that:

- Required metadata is present for processing, and
- Duplicate submissions can be safely ignored.

By combining Zod validation, Turnstile bot checks, and idempotency controls, the React form becomes a **trusted entry point** into the backend pipeline.
...
1. **Turnstile Verification** – Validates the Cloudflare token using the secret stored in **AWS Secrets Manager**.
2. **Payload Validation** – Uses Zod to apply the same schema rules as the frontend, providing an additional line of defense.
3. **Idempotency Check** – Looks up a generated key in DynamoDB to prevent duplicate submissions (e.g., from double-clicking the submit button).
4. **Data Persistence** – Writes the data into **DynamoDB** with an 18-month TTL (time-to-live), ensuring data retention without manual cleanup.
5. **Event Publication** – Emits a `ContactSubmitted` event to **EventBridge**, decoupling the form processing from downstream tasks.
...
It felt great when I finally witnessed the records being successfully written to the database!

<Image 
  src="/blog/images/dynamo-db-success-aws-backend-project.webp" 
  alt="test records written successfully to DynamoDB in AWS" 
/>
...
Right now, the only consumer is the **Email Notifier Lambda**, which listens for `ContactSubmitted` and sends a formatted email through **Resend**, a transactional email service. 

<Image 
  src="/blog/images/resend-email-success-dalton-ousley-portfolio-site.webp" 
  alt="successful email sent to admin from Resend" 
/>

The Resend API key is stored securely in **AWS Secrets Manager**, ensuring no credentials are hard-coded.
...
<Image 
  src="/blog/images/aws-cloudwatch-dashboard-portfolio-site.webp" 
  alt="CloudWatch dashboard in AWS" 
/>
...
Every AWS component, from Lambda functions to EventBridge rules is provisioned using **Terraform**, with configuration files organized by service (`lambda.tf`, `dynamodb.tf`, `eventbridge.tf`, etc.). This modular structure makes the infrastructure easy to navigate, maintain, and version control, while also reinforcing real-world DevOps patterns (I much prefer this over a giant `main.tf` file). 
...
Between Cloudflare Turnstile, API Gateway, Lambda functions, DynamoDB storage, EventBridge events, Secrets Manager integration, and CloudWatch observability — there’s a *lot* happening in the background to make a “simple” form submission secure, scalable, and maintainable. And that’s exactly what I love about cloud engineering: the magic is in the invisible infrastructure.

This project wasn’t without its hiccups either:

- I had early request-validation mismatches that caused failed submissions until I tracked them down through console debug logs paired with CloudWatch logging.
